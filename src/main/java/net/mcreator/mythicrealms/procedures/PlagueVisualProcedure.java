...
public class PlagueVisualProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, RenderPlayerEvent playerRenderEvent) {
		if (entity == null || playerRenderEvent == null)
			return;
		if (entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(MythicrealmsModMobEffects.PLAGUE)) {
			{
				ResourceLocation texture = (ResourceLocation.fromNamespaceAndPath("mythicrealms", "textures/entities/plague_form.png"));
				renderHumanoid(playerRenderEvent, MythicrealmsModHumanoidModels.PLAYER_BASE, playerRenderEvent.getMultiBufferSource().getBuffer(RenderType.entityCutout(texture)), playerRenderEvent.getRenderState());
			}
		}
	}
}